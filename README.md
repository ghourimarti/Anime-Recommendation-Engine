...
**Anime Recommender** is a full-stack, production-grade **conversational anime discovery engine**. You describe
...
> The corpus is the public [MyAnimeList](https://myanimelist.net/) dataset (title, genres,
> synopsis, score) in [`data/anime_with_synopsis.csv`](data/anime_with_synopsis.csv).
...
> **📸 To add these:** create a folder **`docs/screenshots/`** in the repo root and drop your PNGs
> there with the exact filenames below (they're referenced by this README). Recommended width
> ~1400px. Nothing else needs to change — the images will render once the files exist.
...
![Query and streaming recommendations](docs/screenshots/recommend.png)
*Ask in natural language → three grounded recommendation cards → the "why it matches" explanation streams in below.*

### History
![Query history](docs/screenshots/history.png)
*Per-user query history (`GET /v1/history`), scoped to the signed-in Clerk user.*

### Account / Delete My Data
![Account and data deletion](docs/screenshots/account.png)
*Account page with the right-to-be-forgotten flow (`DELETE /v1/account`).*
...
Full walk-through with Mermaid diagrams: **[`docs/architecture.md`](docs/architecture.md)**.
...
├── docs/           # architecture · repo-setup · runbooks/ · hardening/ · deploy/
├── data/anime_with_synopsis.csv           # MAL corpus (catalog of record)
...
### 1 · Install & configure
```bash
git clone <your-repo-url> anime-recommender && cd anime-recommender
...
> cards render and the explanation stream in. Full sequence + troubleshooting:
> **[`docs/run-from-scratch.md`](docs/run-from-scratch.md)**.
...
- **Infrastructure** — IRSA roles scoped to **exact** SQS/S3/secret ARNs (no wildcards), private RDS +
  ...
  NetworkPolicy per namespace. See [`docs/hardening/`](docs/hardening/).
...
| **Load** | **Sustained 50 RPS held** — 15,001 requests, ~0 failures (`tests/load/reports/`). *Scope: the sustained scenario exercises the cached/structured path; full-pipeline latency and the peak/ramp/stream scenarios are in [`tests/load/README.md`](tests/load/README.md).* `TODO: paste your latest peak/stream numbers here` |
...
A clean local → cloud path (see [`docs/deploy/stage1-local-docker.md`](docs/deploy/stage1-local-docker.md)
and [`docs/runbooks/terraform-apply.md`](docs/runbooks/terraform-apply.md)):
...
it healthy. **Anime Recommender** is a deployable service — with tests, an eval gate, security,
